...
public class TodaysPopularproducts extends RecyclerView.Adapter<TodaysPopularproducts.ViewHolder> {
    ...
    private boolean isCart(SQLManager sqlManager, TodaysPopularproducts.ViewHolder holder, int position) {
    ...
    @SuppressLint("UseCompatLoadingForDrawables")
    private boolean isFavorite(SQLManager sqlManager, TodaysPopularproducts.ViewHolder holder, int position){
        //Ürünü veritabanında sorgulamak için ürün ID'sini değiştirilemez şekilde oluşturulması
        final long productId = products.get(position).getProductId();
